...
function getBaseEmptyCells(difficulty: Difficulty): number {
  const defaults: Record<Difficulty, number> = { easy: 30, medium: 40, hard: 50 };
...
export function getEmptyCells(difficulty: Difficulty): number {
  ...
  const isLastLevel = idx >= DIFFICULTY_ORDER.length - 1;
  const max = isLastLevel ? min + 9 : getBaseEmptyCells(DIFFICULTY_ORDER[idx + 1]) - 1;
